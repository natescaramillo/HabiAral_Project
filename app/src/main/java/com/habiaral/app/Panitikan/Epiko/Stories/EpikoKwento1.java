package com.habiaral.app.Panitikan.Epiko.Stories;
...
import com.habiaral.app.Panitikan.Epiko.Quiz.EpikoKwento1Quiz;
...
public class EpikoKwento1 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.panitikan_epiko_kwento1);
        ...
        textToSpeech = new TextToSpeech(this, status -> {
            if (status == TextToSpeech.SUCCESS) {
                ...
                if (result == TextToSpeech.LANG_MISSING_DATA || result == TextToSpeech.LANG_NOT_SUPPORTED) {
                    ...
                    boolean isFirst = prefs.getBoolean("isFirstOpenEpiko1", true);

                    if (isFirst && !isLessonDone) {
                        loadIntroLines();
                        prefs.edit().putBoolean("isFirstOpenEpiko1", false).apply();
                    }
                }
            } else {
        ...
        if (currentPage > 0) {
            ...
            if (currentPage == 0) {
                if (!isLessonDone) {
                    ...
                    introPlayed = false;
                    loadIntroLines();
                    prefs.edit().putBoolean("isFirstOpenEpiko1", true).apply();
                } else {
                    introFinished = true;
                    introPlayed = true;
                }
