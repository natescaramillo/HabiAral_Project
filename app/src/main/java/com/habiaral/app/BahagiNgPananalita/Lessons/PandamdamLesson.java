...
import com.habiaral.app.BahagiNgPananalita.Quiz.PandamdamQuiz;
...
public class PandamdamLesson extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnBack.setOnClickListener(v -> {
            ...
            startActivity(new Intent(PandamdamLesson.this, BahagiNgPananalita.class)
        ...
        unlockButton.setOnClickListener(v -> {
            ...
            stopTTS();
            startActivity(new Intent(this, PandamdamQuiz.class));
        });
    ...
    private void updatePage() {
        ...
        BahagiFirestoreUtils.saveLessonProgress(BahagiFirestoreUtils.getCurrentUser().getUid(),
                "pandamdam", currentPage, isLessonDone);
    ...
    private void checkLessonStatus() {
        ...
        FirebaseFirestore.getInstance().collection("module_progress").document(user.getUid()).get()
                .addOnSuccessListener(snapshot -> {
                    if (snapshot.exists()) {
                        ...
                        if (module1 != null) {
                            ...
                            if (lessons != null && lessons.containsKey("pandamdam")) {
                                Map<String, Object> lessonList = (Map<String, Object>) lessons.get("pandamdam");
                                if (lessonList != null) {
                    ...
                    BahagiFirestoreUtils.saveLessonProgress(user.getUid(), "pandamdam", currentPage, isLessonDone);
                });
    }

    private void loadCharacterLines() {
        FirebaseFirestore.getInstance().collection("lesson_character_lines").document("LCL9").get()
            ...
            @Override public void onDone(String utteranceId) {
                runOnUiThread(() -> {
                    ...
                    if (index[0] < lines.size()) {
                        if (!SoundManagerUtils.isMuted(PandamdamLesson.this)) {
    ...
    private void startIdleGifRandomizer() {
        ...
        idleGifRunnable = new Runnable() {
            ...
            public void run() {
                ...
                if (Math.random() < 0.4) {
                    if (!isFinishing() && !isDestroyed()) {
                        Glide.with(PandamdamLesson.this).asGif().load(R.drawable.right_2).into(imageView2);
                    }
                    idleGifHandler.postDelayed(() -> {
                        if (isActivityActive && imageView2 != null && !isFinishing() && !isDestroyed()) {
                            Glide.with(PandamdamLesson.this).asGif().load(R.drawable.idle).into(imageView2);
                        }
