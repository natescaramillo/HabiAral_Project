...
import com.habiaral.app.BahagiNgPananalita.Quiz.PangawingQuiz;
...
public class PangawingLesson extends AppCompatActivity {
    ...
    private final int[] lessonPPT = {
            R.drawable.pangawing01, R.drawable.pangawing02
    };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnBack.setOnClickListener(v -> {
            ...
            startActivity(new Intent(PangawingLesson.this, BahagiNgPananalita.class)
    ...
    private void checkLessonStatus() {
        ...
        FirebaseFirestore.getInstance().collection("module_progress").document(user.getUid()).get()
                .addOnSuccessListener(snapshot -> {
                    if (snapshot.exists()) {
                        ...
                        if (module1 != null) {
                            ...
                            if (lessons != null && lessons.containsKey("pangawing")) {
                                Map<String, Object> lessonList = (Map<String, Object>) lessons.get("pangawing");
                                if (lessonList != null) {
                    ...
                    BahagiFirestoreUtils.saveLessonProgress(user.getUid(), "pangawing", currentPage, isLessonDone);
                });
    }

    private void loadCharacterLines() {
        FirebaseFirestore.getInstance().collection("lesson_character_lines").document("LCL10").get()
    ...
    private void startIdleGifRandomizer() {
        ...
        idleGifRunnable = new Runnable() {
            ...
            public void run() {
                ...
                if (Math.random() < 0.4) {
                    if (!isFinishing() && !isDestroyed()) {
                        Glide.with(PangawingLesson.this).asGif().load(R.drawable.right_2).into(imageView2);
                    }
                    idleGifHandler.postDelayed(() -> {
                        if (isActivityActive && imageView2 != null && !isFinishing() && !isDestroyed()) {
                            Glide.with(PangawingLesson.this).asGif().load(R.drawable.idle).into(imageView2);
                        }
