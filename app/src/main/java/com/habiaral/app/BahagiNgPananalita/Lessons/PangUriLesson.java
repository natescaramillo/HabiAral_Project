...
import com.habiaral.app.BahagiNgPananalita.Quiz.PangUriQuiz;
                    ...
                    if (snapshot.exists()) {
                        ...
                        if (module1 != null) {
                            ...
                            if (lessons != null && lessons.containsKey("panguri")) {
                                Map<String, Object> lessonList = (Map<String, Object>) lessons.get("panguri");
                                if (lessonList != null) {
    ...
    private void startIdleGifRandomizer() {
        ...
        idleGifRunnable = new Runnable() {
            ...
            public void run() {
                ...
                if (Math.random() < 0.4) {
                    if (!isFinishing() && !isDestroyed()) {
                        Glide.with(PangUriLesson.this).asGif().load(R.drawable.right_2).into(imageView2);
                    }
                    idleGifHandler.postDelayed(() -> {
                        if (isActivityActive && imageView2 != null && !isFinishing() && !isDestroyed()) {
                            Glide.with(PangUriLesson.this).asGif().load(R.drawable.idle).into(imageView2);
                        }
