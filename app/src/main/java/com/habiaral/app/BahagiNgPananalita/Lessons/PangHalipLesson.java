...
import com.habiaral.app.BahagiNgPananalita.Quiz.PangHalipQuiz;
                    ...
                    if (snapshot.exists()) {
                        ...
                        if (module1 != null) {
                            ...
                            if (lessons != null && lessons.containsKey("panghalip")) {
                                Map<String, Object> lessonList = (Map<String, Object>) lessons.get("panghalip");
                                if (lessonList != null) {
            ...
            @Override public void onDone(String utteranceId) {
                runOnUiThread(() -> {
                    ...
                    if (index[0] < lines.size()) {
                        if (!SoundManagerUtils.isMuted(PangHalipLesson.this)) {
    ...
    private void startIdleGifRandomizer() {
        ...
        idleGifRunnable = new Runnable() {
            ...
            public void run() {
                ...
                if (Math.random() < 0.4) {
                    if (!isFinishing() && !isDestroyed()) {
                        Glide.with(PangHalipLesson.this).asGif().load(R.drawable.right_2).into(imageView2);
                    }
                    idleGifHandler.postDelayed(() -> {
                        if (isActivityActive && imageView2 != null && !isFinishing() && !isDestroyed()) {
                            Glide.with(PangHalipLesson.this).asGif().load(R.drawable.idle).into(imageView2);
                        }
