...
import com.habiaral.app.BahagiNgPananalita.Quiz.PangAngkopQuiz;
...
public class PangAngkopLesson extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        unlockButton.setOnClickListener(v -> {
            ...
            stopTTS();
            startActivity(new Intent(this, PangAngkopQuiz.class));
        });
    ...
    private void checkLessonStatus() {
        ...
        FirebaseFirestore.getInstance().collection("module_progress").document(user.getUid()).get()
                .addOnSuccessListener(snapshot -> {
                    if (snapshot.exists()) {
                        ...
                        if (module1 != null) {
                            ...
                            if (lessons != null && lessons.containsKey("pangangkop")) {
                                Map<String, Object> lessonList = (Map<String, Object>) lessons.get("pangangkop");
                                if (lessonList != null) {
                    ...
                    BahagiFirestoreUtils.saveLessonProgress(user.getUid(), "pangangkop", currentPage, isLessonDone);
                });
    }

    private void loadCharacterLines() {
        FirebaseFirestore.getInstance().collection("lesson_character_lines").document("LCL8").get()
            ...
            @Override public void onDone(String utteranceId) {
                runOnUiThread(() -> {
                    ...
                    if (index[0] < lines.size()) {
                        if (!SoundManagerUtils.isMuted(PangAngkopLesson.this)) {
    ...
    private void startIdleGifRandomizer() {
        ...
        idleGifRunnable = new Runnable() {
            ...
            public void run() {
                ...
                if (Math.random() < 0.4) {
                    if (!isFinishing() && !isDestroyed()) {
                        Glide.with(PangAngkopLesson.this).asGif().load(R.drawable.right_2).into(imageView2);
                    }
                    idleGifHandler.postDelayed(() -> {
                        if (isActivityActive && imageView2 != null && !isFinishing() && !isDestroyed()) {
                            Glide.with(PangAngkopLesson.this).asGif().load(R.drawable.idle).into(imageView2);
                        }
