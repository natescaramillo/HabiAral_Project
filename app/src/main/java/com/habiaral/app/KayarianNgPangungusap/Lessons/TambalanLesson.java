...
import com.habiaral.app.KayarianNgPangungusap.Quiz.TambalanQuiz;
    ...
    private void startIdleGifRandomizer() {
        ...
        idleGifRunnable = new Runnable() {
            ...
            public void run() {
                ...
                if (Math.random() < 0.4) {
                    if (!isFinishing() && !isDestroyed()) {
                        Glide.with(TambalanLesson.this).asGif().load(R.drawable.right_2).into(imageView2);
                    }
                    idleGifHandler.postDelayed(() -> {
                        if (isActivityActive && imageView2 != null && !isFinishing() && !isDestroyed()) {
                            Glide.with(TambalanLesson.this).asGif().load(R.drawable.idle).into(imageView2);
                        }
        ...
        db.collection("module_progress").document(uid).get()
                .addOnSuccessListener(documentSnapshot -> {
                        ...
                        if (module2 != null) {
                            ...
                            if (lessons != null) {
                                ...
                                if (tambalan != null) {
                                    ...
                                    String introStatus = (String) tambalan.get("intro");
                    ...
                    if (!alreadyCompleted) {
                        ...
                        Map<String, Object> tambalanMap = new HashMap<>();
                        tambalanMap.put("status", "in_progress");

                        Map<String, Object> lessonsMap = new HashMap<>();
                        lessonsMap.put("tambalan", tambalanMap);

                        Map<String, Object> module2Map = new HashMap<>();
                        module2Map.put("lessons", lessonsMap);
                        module2Map.put("current_lesson", "tambalan");
