...
public class PangatnigLesson extends AppCompatActivity {
    ...
    private final int[] pangatnigLesson = {
                    ...
                    if (snapshot.exists()) {
                        ...
                        if (module1 != null) {
                            ...
                            if (lessons != null && lessons.containsKey("pangatnig")) {
                                ...
                                if (pangatnig != null) {
                                    Long checkpoint = (Long) pangatnig.get("checkpoint");
                                    currentPage = (checkpoint != null) ? checkpoint.intValue() : 0;
                                    isFirstTime = false;

                                    if ("completed".equals(pangatnig.get("status"))) {
                                        isLessonDone = true;
                                        unlockButton.setEnabled(true);
                                        unlockButton.setAlpha(1f);
                                    }
    ...
    private void saveProgressToFirestore(boolean completed) {
        ...
        pangatnigStatus.put("status", statusToSave);
        pangatnigStatus.put("checkpoint", currentPage);
    ...
    private void loadCharacterLines() {
        ...
        db.collection("lesson_character_lines").document("LCL6")
                .get()
