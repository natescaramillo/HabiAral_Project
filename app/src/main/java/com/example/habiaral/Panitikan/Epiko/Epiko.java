package com.example.habiaral.Panitikan.Epiko;
...
import com.example.habiaral.Panitikan.Epiko.Stories.EpikoKwento1;
...
public class Epiko extends AppCompatActivity {
    ...
    private void markCategoryInProgressIfNeeded() {
        ...
        db.collection("module_progress").document(uid).get()
                .addOnSuccessListener(snapshot -> {
                    ...
                    if (categories != null && categories.get("Epiko") instanceof Map) {
                        Map<String, Object> epikoCat = (Map<String, Object>) categories.get("Epiko");
                        String status = (String) epikoCat.get("status");
                        if ("completed".equals(status)) return;
                    }

                    Map<String, Object> categoryUpdate = new HashMap<>();
                    categoryUpdate.put("categoryname", "Epiko");
                    categoryUpdate.put("status", "in_progress");

                    db.collection("module_progress").document(uid)
                            .set(Map.of("module_3", Map.of("categories", Map.of("Epiko", categoryUpdate))),
                                    SetOptions.merge());
                });
    }
    ...
    private void updateUIFromProgress(Map<String, Object> data) {
        ...
        Map<String, Object> epikoCat = (Map<String, Object>) categories.get("Epiko");
        if (epikoCat == null) return;

        Map<String, Object> stories = (Map<String, Object>) epikoCat.get("stories");
        if (stories == null) return;

        boolean kwento1Done = isCompleted(stories, "EpikoKwento1");
        boolean kwento2Done = isCompleted(stories, "EpikoKwento2");
    ...
    private void initViews() {
        ...
        btnKwento1.setOnClickListener(v -> openStory(EpikoKwento1.class));
        btnKwento2.setOnClickListener(v -> openStory(EpikoKwento2.class));
    }
