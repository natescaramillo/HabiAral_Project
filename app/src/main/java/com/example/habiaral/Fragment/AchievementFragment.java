            ...
            for (int i = 0; i < boxIds.length && i < achievementDocs.size(); i++) {
                ...
                if (achievementsMap.containsKey(saKey)) {
                    ...
                    if (saData != null && saData.get("unlockedAt") instanceof Timestamp) {
                        ...
                        unlockedAtStr = formatDateToTagalog(ts.toDate());
                    }
                    ...
                    image.setAlpha(0.3f);
                    addLockIcon(image);
                    box.setOnClickListener(v -> Toast.makeText(getContext(), "Hindi mo pa nakakamit ang gantimpalang ito.", Toast.LENGTH_SHORT).show());
                }
            }
        }).addOnFailureListener(e -> Log.e("AchievementFragment", "Error loading unlocked achievements", e));
    }
    private String formatDateToTagalog(Date date) {
        String[] buwan = {
        ...
        int araw = cal.get(Calendar.DAY_OF_MONTH);
        int buwanIndex = cal.get(Calendar.MONTH);
        int taon = cal.get(Calendar.YEAR);

        return araw + " " + buwan[buwanIndex] + " " + taon;
    }
