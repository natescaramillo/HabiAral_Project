...
//            stopTTS();
            startActivity(new Intent(this, HugnayanQuiz.class));
        });
...
//                "hugnayan", currentPage, isLessonDone);
//
//        stopSpeaking();
                ...
                .addOnSuccessListener(snapshot -> {
                    if (snapshot.exists()) {
                        ...
                        if (module1 != null) {
                            ...
                            if (lessons != null && lessons.containsKey("hugnayan")) {
                                Map<String, Object> hugnayan = (Map<String, Object>) lessons.get("hugnayan");
                                if (hugnayan != null) {
                                    Long checkpoint = (Long) hugnayan.get("checkpoint");
                                    currentPage = (checkpoint != null) ? checkpoint.intValue() : 0;
                                    isFirstTime = false;
                                    if ("completed".equals(hugnayan.get("status"))) {
                                        isLessonDone = true;
                                        unlockButton.setEnabled(true);
                                        unlockButton.setAlpha(1f);
                                    }
...
//                        imageView.setImageResource(pangngalanLesson[currentPage]);
                    ...
                    KayarianFirestoreUtils.saveLessonProgress(user.getUid(), "hugnayan", currentPage, isLessonDone);
                });
    }
...
//                        if (!SoundManager.isMuted(HugnayanLesson.this)) {
